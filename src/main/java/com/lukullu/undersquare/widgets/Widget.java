...
public class Widget implements ProcessingClass {
	...
	public Widget(Vector2 _pos, Vector2 _dim, int _cornerBL, int _cornerBR, int _cornerTL, int _cornerTR, Color _color) {
		pos = _pos; dim = _dim; cornerBL = _cornerBL; cornerBR = _cornerBR; cornerTL = _cornerTL; cornerTR = _cornerTR; c = _color;}
	
	// methods that take their relative position
	// if you don't need the vec2, simply ignore it.
	public void update(Vector2 _rel) {}
	public void paint(Vector2 _rel) { noStroke(); fill(c.getRGB()); rect(pos.x + _rel.x, pos.y + _rel.y, dim.x, dim.y, cornerTL, cornerTR, cornerBR, cornerBL);}
